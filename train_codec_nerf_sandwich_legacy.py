...
from src.models import STE_DVGO_Image
...
class Trainer:
    ...
    def _build_model(self):
        ...
        if self.cfg.codec.train_mode == 'ste':
            self.model = STE_DVGO_Image(ids, xyz_min, xyz_max, self.cfg).to(self.device)
        else:
    ...
    def _train_step(self, step: int):
        ...
        # ---- NEW: identity loss for sandwich pre/post (codec-free) ----
        id_every = int(getattr(self.cfg.fine_train, "sandwich_id_every", 1))
        do_sandwich_id = (id_every <= 1) or ((step % id_every) == 0)

        if do_sandwich_id and getattr(self.cfg.codec, "use_sandwich", False) and \
        float(getattr(self.cfg.codec, "weight_sandwich_id", 0.0)) > 0.0:
            frame_id = int(torch.unique(fid_b).item()) # batch contains one frame id
            sandwich_id_loss = self._sandwich_identity_loss(frame_id)
        else:
            sandwich_id_loss = torch.tensor(0.0, device=self.device)

        # Backprop
        self.optimizer.zero_grad(set_to_none=True)
        loss = rec_loss + bpp_loss + sandwich_id_loss
        loss.backward()
        ...
        self.optimizer.step()
        return loss, psnr_by_axis, avg_bpp, rec_loss, bpp_loss, sandwich_id_loss
    ...
    def train(self):
        ...
        for step in trange(1, cfg_t.N_iters+1):
            ...
            loss, plane_psnr_dict, avg_bpp, rec_loss, bpp_loss, sandwich_id_loss = self._train_step(step)
            ...
            if step % self.args.i_print == 0 or step == 1:
                ...
                tqdm.write(f'[step {step:6d}] loss {loss.item():.4e} psnr {psnr:5.2f} s_loss {sandwich_id_loss.item():.4e} '
            ...
            if step % self.args.i_log == 0 and WANDB:
                wandb.log({
                    ...
                    "train/sandwich_id_loss": float(sandwich_id_loss.item()),
